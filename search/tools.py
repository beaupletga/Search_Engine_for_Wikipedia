...
def get_vector_word2vec(text_split):
    filtered_sentence = [model.wv[w] for w in text_split if (not w in stop_words and model.wv.__contains__(w))]
...
def get_closest_articles(C,query,k=10):
    ...
    numerator = (C[:,keys]* np.array(tf_idf).reshape(len(tf_idf),1)).sum(axis=1)
    # denominator = np.sqrt(C[:,keys].power(2).sum(axis=1))*norm_query
    dist = (numerator.reshape(-1))#/denominator).reshape(-1)
    argsort_var=np.flip(np.argsort(dist))
    # print(dist.shape,argsort_var.shape)
    result_dataset=[]
    conn = sqlite3.connect(path+'Database.db')
    c = conn.cursor()
    for i in range(k):
        if dist[argsort_var[i]]!=0:
    ...
    return result_dataset,[dist[argsort_var[i]] for i in range(len(result_dataset))]
...
def get_page_rank(id_article_list):
    ...
    # print(dist_pr)
    dist_pr=[np.round(i/np.max(dist_pr),2) for i in dist_pr]
    conn.close()
    return dist_pr
...
def get_correct_sentence(sentence):
    ...
    for word in mispelled_words:
        ...
        reco = set([item for sublist in l for item in sublist if is_in(item)])
        dict1 = Counter(word) 
        reco_list=[]
        for i in reco:
            dict2 = Counter(i)
